...
def tema_degistir(): st.session_state.tema = st.session_state.get("tema_secimi","Koyu")
...
def enerjik_css(tema):
    ...
    metric_bg = "linear-gradient(145deg, #1a1f35, #0f1424)" if tema=="Koyu" else "linear-gradient(145deg, #E2E8F0, #CBD5E1)"
    ...
    <style>
        ...
        div[data-testid="stMetric"] {{ background: {metric_bg}; border: 1px solid #94A3B8; border-radius: 20px; padding: 24px; color: {text}; box-shadow: 0 10px 25px rgba(0,0,0,0.1); }}
        h1,h2,h3,h4,h5,p,span,label {{ color: {text}; }}
        .stButton > button {{ border-radius: 14px; font-weight: 700; background: linear-gradient(135deg, #F97316, #8B5CF6); color: white; border: none; padding: 0.7rem 2rem; }}
        input,select,textarea {{ background-color: {input_bg} !important; color: {text} !important; border: 1px solid #94A3B8 !important; border-radius: 10px !important; }}
        .main-header {{ font-size: 2.3rem; font-weight: 800; background: linear-gradient(135deg, #F97316, #8B5CF6); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin-bottom: 1.5rem; padding-bottom: 0.5rem; border-bottom: 2px solid #F97316; }}
        @media (max-width: 768px) {{ .main-header {{ font-size: 1.4rem !important; }} .stButton > button {{ width: 100% !important; }} }}
    </style>
    ...
    if os.path.exists(dosya_adi):
        try:
            with open(dosya_adi,"r",encoding="utf-8") as f:
                return json.load(f)
        except Exception as e: logging.error(f"Dosya okuma hatası {dosya_adi}: {e}")
...
def get_font_path():
    ...
    logging.warning("DejaVuSans.ttf bulunamadı, Helvetica kullanılacak (Türkçe karakter sorunu olabilir).")
    return None
...
def fis_olustur(urun_adi, birim, miktar, birim_fiyat, toplam_tutar, odeme_tipi):
    ...
    pdf.cell(50,8,txt="Ürün:",border=0); pdf.cell(30,8,txt=urun_adi[:20],border=0,ln=True)
    pdf.cell(50,8,txt="Miktar:",border=0); pdf.cell(30,8,txt=f"{miktar} {birim}",border=0,ln=True)
    pdf.cell(50,8,txt="Birim Fiyat:",border=0); pdf.cell(30,8,txt=f"{birim_fiyat:.2f} ₺",border=0,ln=True)
    pdf.ln(3)
    pdf.set_font("DejaVu" if font_path else "Helvetica",size=12)
    pdf.cell(50,10,txt="TOPLAM:",border=0); pdf.cell(30,10,txt=f"{toplam_tutar:.2f} ₺",border=0,ln=True)
    pdf.set_font("DejaVu" if font_path else "Helvetica",size=8)
...
def veri_gecis_kontrol():
    ...
    for u in st.session_state.stok:
        for k,v in [("kategori","Diğer"),("min_miktar",0),("barkod",""),("son_kullanma_tarihi",""),("alis_fiyat",0),
                    ("satis_fiyat",0),("tedarikci",""),("raf_no",""),("kdv_oran",8),("tahmini_gunluk_satis",1.0)]:
...
def mock_stok_olustur():
    return [
        {"urun_adi":"Un","miktar":150,"birim":"kg","kategori":"Kuru Gıda","min_miktar":20,"barkod":"8691234567890",
         "son_kullanma_tarihi":"2026-12-31","alis_fiyat":18.5,"satis_fiyat":25.9,"tedarikci":"ABC Un","kdv_oran":1,
         "raf_no":"A1","tahmini_gunluk_satis":5.0},
        {"urun_adi":"Şeker","miktar":5,"birim":"kg","kategori":"Kuru Gıda","min_miktar":10,"barkod":"8691234567891",
         "son_kullanma_tarihi":"2026-05-15","alis_fiyat":22,"satis_fiyat":32.5,"tedarikci":"XYZ Şeker","kdv_oran":8,
         "raf_no":"A2","tahmini_gunluk_satis":2.0},
        {"urun_adi":"Süt","miktar":40,"birim":"litre","kategori":"Süt Ürünleri","min_miktar":15,"barkod":"8691234567892",
         "son_kullanma_tarihi":"2026-05-08","alis_fiyat":12,"satis_fiyat":18.9,"tedarikci":"Sütaş","kdv_oran":1,
         "raf_no":"B1","tahmini_gunluk_satis":8.0},
        {"urun_adi":"Yumurta","miktar":200,"birim":"adet","kategori":"Diğer","min_miktar":30,"barkod":"8691234567893",
         "son_kullanma_tarihi":"2026-05-06","alis_fiyat":2.5,"satis_fiyat":4.5,"tedarikci":"Köy Yumurtası","kdv_oran":1,
         "raf_no":"C1","tahmini_gunluk_satis":30.0},
        {"urun_adi":"Tereyağı","miktar":25,"birim":"kg","kategori":"Süt Ürünleri","min_miktar":5,"barkod":"8691234567894",
         "son_kullanma_tarihi":"2026-06-20","alis_fiyat":120,"satis_fiyat":175,"tedarikci":"Sütaş","kdv_oran":8,
         "raf_no":"B2","tahmini_gunluk_satis":3.0},
    ]

def mock_fire_olustur():
    return [
        {"urun_adi":"Un","miktar":10,"birim":"kg","aciliyet":"🔥 Yüksek","tedarikci":"ABC Un","durum":"Bekliyor","eklenme_tarihi":datetime.now().strftime("%Y-%m-%d %H:%M")},
...
def satis_kaydet(ad,birim,miktar,fiyat,tutar,kul,alis_fiyat=0,odeme_tipi="Nakit"):
    s = {"id":str(uuid.uuid4())[:8],"tarih":datetime.now().strftime("%Y-%m-%d %H:%M:%S"),"kullanici":kul,
         "urun_adi":ad,"birim":birim,"miktar":miktar,"birim_fiyat":fiyat,"toplam_tutar":tutar,
         "alis_fiyat":alis_fiyat,"maliyet":round(miktar*alis_fiyat,2),"odeme_tipi":odeme_tipi}
...
def gunluk_kar():
    ...
    toplam_maliyet = sum(s.get("maliyet", s["miktar"]*s.get("alis_fiyat",0)) for s in liste if s["tarih"].startswith(bugun))
...
def haftalik_ciro(baslangic=None):
    if baslangic is None: baslangic = datetime.now().date() - timedelta(days=datetime.now().weekday())
...
def email_gonder(alici, konu, mesaj):
    try:
        ...
        baglanti.quit()
        return True
    except Exception as e: logging.error(f"E-posta hatası: {e}"); return False
...
def email_gonder_pdf(alici, konu, mesaj, pdf_bytes, dosya_adi):
    try:
        ...
        baglanti.quit()
        return True
    except Exception as e: logging.error(f"PDF e-posta hatası: {e}"); return False
...
def urun_gunluk_satis_hizi(urun_adi, varsayilan=1.0):
    ...
    for s in satislar:
        try:
            tarih = datetime.strptime(s["tarih"],"%Y-%m-%d %H:%M:%S").date()
            if s["urun_adi"]==urun_adi and baslangic<=tarih<=bugun: miktarlar.append(s["miktar"])
        except: continue
    if not miktarlar:
        for u in st.session_state.stok:
            if u["urun_adi"]==urun_adi: return u.get("tahmini_gunluk_satis",varsayilan)
...
def bilimsel_indirim_hesapla(urun, kalan_gun):
    if kalan_gun<=0:
        ...
        return 50
    q = urun.get("miktar",0); satis_fiyat = urun.get("satis_fiyat",0); alis_fiyat = urun.get("alis_fiyat",0)
...
def otomatik_yedekleme_kontrol():
    ...
    if gonder:
        yedek = {"stok":st.session_state.stok,"fire":st.session_state.fire,"barkod_db":st.session_state.barkod_db,
                 "tedarikciler":st.session_state.tedarikciler,"tarih":datetime.now().isoformat()}
    ...
    with col2:
        ...
        with st.form("giris"):
            ...
            if st.form_submit_button("🚀 Giriş Yap", use_container_width=True):
                try: admin_user = st.secrets["admin"]["kullanici_adi"]; admin_pass = st.secrets["admin"]["sifre"]
                except: admin_user = config.get("kullanici_adi","admin"); admin_pass = config.get("sifre","1234")
...
def izinli_sayfalar(kullanici):
    ...
    yetki_sayfa = {"barkod":["📱 Barkod","🏷️ Barkod Yönetimi"],"satis":["💵 Satış"],"stok":["📦 Stok","📈 Stok Analizi","📉 Fire Analizi"],"stok_ekle":["📦 Stok","🔥 Sipariş"],"skt_takip":["🏠 Ana Panel"]}
    return {k:v for k,v in SAYFALAR.items() if any(k in yetki_sayfa.get(yetki,[]) for yetki in izinler)}
...
def ana_sayfa():
    ...
    if kritik: st.balloons(); st.error(f"🚨 {len(kritik)} ürün kritik stok seviyesinde!")
    bugun = datetime.now()
    ...
    c2.metric("Bugünkü Kâr",f"{kar:,.0f} ₺"); c3.metric("Dünkü Ciro",f"{dun:,.0f} ₺")
...
def barkod_yonetimi():
    ...
    with tab1:
        if st.session_state.barkod_db:
            ...
            if arama: df = df[df["barkod"].str.contains(arama,case=False,na=False) | df["urun_adi"].str.contains(arama,case=False,na=False)]
            ...
            for _,row in df.iterrows():
                ...
                with st.expander(f"🔧 {row['urun_adi']} ({bk})"):
                    ...
                    with col1:
                        ...
                        yeni_birim = st.selectbox("Birim",BIRIMLER,index=BIRIMLER.index(row["birim"]) if row["birim"] in BIRIMLER else 0,key=f"birim_{bk}")
                        yeni_kat = st.selectbox("Kategori",KATEGORILER,index=KATEGORILER.index(row["kategori"]) if row["kategori"] in KATEGORILER else 0,key=f"kat_{bk}")
                        yeni_uret = st.text_input("Üretici",row.get("uretici",""),key=f"uret_{bk}")
                    with col2:
                        if st.button("💾 Güncelle",key=f"guncelle_{bk}"):
                            st.session_state.barkod_db[bk].update({"urun_adi":yeni_ad,"birim":yeni_birim,"kategori":yeni_kat,"uretici":yeni_uret})
                        ...
                        if st.button("🗑️ Sil",key=f"sil_{bk}"):
                            if any(u.get("barkod")==bk for u in st.session_state.stok):
                                st.error("Bu barkod stokta kullanılıyor, önce ürünü silin veya barkodunu değiştirin.")
                            else:
                                del st.session_state.barkod_db[bk]
                                dosya_yaz(BARKOD_DB_DOSYASI,st.session_state.barkod_db)
                                st.session_state.son_islem_mesaji = f"✅ Barkod {bk} silindi"; st.rerun()
        else: st.info("Barkod veritabanı boş.")
    with tab2:
        with st.form("yeni_barkod"):
            ...
            if st.form_submit_button("Ekle"):
                if not yeni_barkod or not yeni_ad: st.error("Barkod ve ürün adı zorunlu")
                elif yeni_barkod in st.session_state.barkod_db: st.error("Barkod zaten var")
                else:
                    st.session_state.barkod_db[yeni_barkod] = {"urun_adi":yeni_ad,"birim":yeni_birim,"kategori":yeni_kat,"uretici":""}
                    dosya_yaz(BARKOD_DB_DOSYASI,st.session_state.barkod_db)
...
def barkod_sayfasi():
    ...
    col1,col2 = st.columns(2)
    with col1: barkod_manuel = st.text_input("🔢 Barkod Numarası", placeholder="Okutun veya yazın...", key="manuel_barkod")
    with col2: img_file = st.camera_input("📷 Mobil Kamera")
    barkod = None
    if img_file:
        try:
    ...
    if aktif:
        ...
        if urun_adi: st.info(f"📦 **{guvenli_html(urun_adi)}** ({bilgi.get('birim','')})")
        ...
        with st.form("barkod_form"):
            ...
            birim = st.selectbox("Birim",BIRIMLER, index=BIRIMLER.index(bilgi.get("birim","adet")) if bilgi.get("birim") in BIRIMLER else 0)
            kategori = st.selectbox("Kategori",KATEGORILER, index=KATEGORILER.index(bilgi.get("kategori","Diğer")) if bilgi.get("kategori") in KATEGORILER else 0)
            ...
            if urun:
                with cols[i]:
                    if st.button(f"🛒 {urun['urun_adi']}\n1 {urun['birim']}", key=f"hizli_{ad}"):
                        urun["miktar"] -= 1
                        satis_kaydet(urun["urun_adi"], urun["birim"], 1, urun.get("satis_fiyat",0), urun.get("satis_fiyat",0),
                                     st.session_state.current_user["kullanici_adi"], urun.get("alis_fiyat",0), "Nakit")
                        veriyi_kaydet(); st.session_state.son_islem_mesaji = f"✅ Hızlı satış: {urun['urun_adi']}"; st.rerun()
                    ...
                    if u["miktar"]<=u.get("min_miktar",0) and u.get("min_miktar",0)>0:
                        if not any(f["urun_adi"]==u["urun_adi"] and f["durum"]=="Bekliyor" for f in st.session_state.fire):
                            st.session_state.fire.append({
                                "urun_adi":u["urun_adi"], "miktar":u["min_miktar"]-u["miktar"]+2, "birim":u["birim"],
                                "aciliyet":"🔥 Yüksek","tedarikci":u.get("tedarikci",""),"durum":"Bekliyor",
    ...
    if st.session_state.pos_sepet:
        ...
        for bk, adet in st.session_state.pos_sepet.items():
            ...
            if urun:
                ...
                col1.write(f"📦 {urun['urun_adi']} – {adet} x {fiyat:.2f} = {tutar:.2f} ₺")
                col2.number_input("Adet",1,value=adet,key=f"adet_{bk}", on_change=lambda b=bk: st.session_state.pos_sepet.update({b: st.session_state[f"adet_{b}"]}))
                if col3.button("🗑️",key=f"sil_{bk}"): del st.session_state.pos_sepet[bk]; st.rerun()
        ...
        if st.button("Satışı Tamamla"):
            for bk, adet in st.session_state.pos_sepet.items():
                ...
                if urun and urun["miktar"]>=adet:
                    urun["miktar"] -= adet
                    satis_kaydet(urun["urun_adi"], urun["birim"], adet, urun.get("satis_fiyat",0), adet*urun.get("satis_fiyat",0),
                                 st.session_state.current_user["kullanici_adi"], urun.get("alis_fiyat",0), "Nakit")
            ...
            st.session_state.pos_sepet = {}
            st.success(f"Satış tamamlandı! Toplam: {toplam_tutar:.2f} ₺"); st.rerun()
            ...
            if st.form_submit_button("Kaydet"):
                if not ad.strip(): st.error("Ad zorunlu")
                    ...
                    if barkod and barkod not in st.session_state.barkod_db:
                        ...
                        dosya_yaz(BARKOD_DB_DOSYASI,st.session_state.barkod_db)
                    st.session_state.stok.append({"urun_adi":ad.strip(),"miktar":miktar,"birim":birim,"kategori":kategori,"min_miktar":min_m,"barkod":barkod,"son_kullanma_tarihi":skt.strftime("%Y-%m-%d") if skt_var else "","alis_fiyat":alis,"satis_fiyat":satis,"raf_no":"","tahmini_gunluk_satis":1.0})
                    veriyi_kaydet(); st.session_state.son_islem_mesaji = f"🎉 {ad.strip()} eklendi!"; st.rerun()
    with tab3:
        if st.session_state.stok:
            ...
            with st.form("duzenle_form"):
                ...
                yeni_birim = st.selectbox("Birim",BIRIMLER, index=BIRIMLER.index(urun.get("birim","adet")) if urun.get("birim") in BIRIMLER else 0)
                yeni_kat = st.selectbox("Kategori",KATEGORILER, index=KATEGORILER.index(urun.get("kategori","Diğer")) if urun.get("kategori") in KATEGORILER else 0)
                ...
                if st.form_submit_button("Güncelle"):
                    urun.update({"urun_adi":yeni_ad,"miktar":yeni_miktar,"birim":yeni_birim,"kategori":yeni_kat,"alis_fiyat":yeni_alis,"satis_fiyat":yeni_satis,"min_miktar":yeni_min})
                    veriyi_kaydet(); st.session_state.son_islem_mesaji = f"✅ {yeni_ad} güncellendi"; st.rerun()
        ...
        if sayim_barkod:
            ...
            if urun:
                ...
                if st.button("Sayımı Kaydet"):
                    ...
                    urun['miktar'] = yeni
                    veriyi_kaydet()
                    hareket_ekle(st.session_state.current_user["kullanici_adi"],"Sayım Düzeltme",urun['urun_adi'],f"Fark: {fark:+.2f}")
                    st.session_state.son_islem_mesaji = f"✅ {urun['urun_adi']} sayımı kaydedildi"; st.rerun()
            else: st.warning("Barkod stokta yok.")
    with tab5:
                ...
                for _,row in df.iterrows():
                    for u in st.session_state.stok:
                        if u.get("barkod")==str(row["barkod"]).strip():
                            u["miktar"] += float(row["miktar"])
                            break
                veriyi_kaydet(); st.session_state.son_islem_mesaji = f"✅ {len(df)} ürün güncellendi"; st.rerun()
...
def siparis_sayfasi():
    ...
    with tab1:
        for i,f in enumerate(st.session_state.fire):
            ...
            if c2.button("🗑️", key=f"sil_fire_{i}"): st.session_state.fire.pop(i); veriyi_kaydet(); st.rerun()
            if c3.button("📧", key=f"mail_fire_{i}"):
                t_eposta = next((t["eposta"] for t in st.session_state.tedarikciler if t["ad"]==f.get("tedarikci","")), "")
                if t_eposta:
                    urun_dict = f.copy()
                    for u in st.session_state.stok:
                        if u["urun_adi"]==f["urun_adi"]: urun_dict["miktar"]=u["miktar"]; urun_dict["min_miktar"]=u.get("min_miktar",10); break
                    tedarikciye_siparis_gonder(urun_dict, t_eposta)
                    st.session_state.son_islem_mesaji = f"📧 {f['urun_adi']} siparişi gönderildi"; st.rerun()
                else: st.error("Tedarikçi e-postası yok")
    with tab2:
        with st.form("fire_ekle"):
            if not st.session_state.stok: st.warning("Önce ürün ekleyin")
            else:
                ...
                if st.form_submit_button("Ekle"):
                    st.session_state.fire.append({
                        "urun_adi":urun_sec,"miktar":miktar,"birim":urun_bilgi['birim'],"aciliyet":"⚡ Orta",
                        "tedarikci":urun_bilgi.get("tedarikci",""),"durum":"Bekliyor",
                        "eklenme_tarihi":datetime.now().strftime("%Y-%m-%d %H:%M")
                    })
...
def kullanici_yonetimi():
    ...
    with st.form("kullanici_ekle"):
        ...
        if st.form_submit_button("Ekle"):
            ...
            else: st.error("Kullanıcı adı ve şifre zorunlu")
...
def sifre_sifirla():
    ...
    with st.form("sifre_sifirla"):
        ...
        if st.form_submit_button("Sıfırla"):
            ...
            else: config["sifre"]=yeni; dosya_yaz(CONFIG_DOSYASI,config); st.session_state.son_islem_mesaji="Şifre güncellendi"; st.rerun()
